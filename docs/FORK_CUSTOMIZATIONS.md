...
> 状态：active
> 当前验证基线：`upstream/main@accb918eca222e35f0648aeb58535868825af723`
> 对应合并提交：`b890872418b6d4f8c296dc845a6b83e7a884c3b1`
> 最后核验：2026-07-15
...
- 产品目的：极速蹬生产始终从 `origin/play/main` 触发 Zeabur 构建。
- 不变量：同步使用 `sync/upstream-YYYYMMDD` merge 分支，禁止 rebase `play/main`；部署脚本拒绝推送 `main`；验收地址为 `https://www.jisudeng.com/`；禁止用本地服务作为验收结论。
- 关键位置：`scripts/push-github-and-deploy.sh`、`deploy/zeabur.template.yaml`、`.cursor/rules/sub2api-server-only-verify.mdc`、[上游同步手册](./UPSTREAM_SYNC_PLAYBOOK.md)。
- 冲突策略：上游 Docker 文档只作为参考，不能改变极速蹬生产分支和验收入口。
- 验证：integrity 脚本、GitHub CI、Zeabur 构建状态和生产冒烟检查。
...
- 不变量：扣费前验证 API Key 与用户归属；订阅扣费验证订阅所有者；余额冻结同样验证归属；模型目录参考价不能覆盖真实渠道计费；支付订单完成后再授予 recharge boost，boost 失败不得回滚已完成充值。
- 关键位置：`backend/internal/repository/usage_billing_repo.go`、`backend/internal/service/gateway_usage_billing.go`、`backend/internal/service/payment_fulfillment.go`、`backend/internal/service/play_recharge_boost.go`。
- 冲突策略：上游支付状态机和安全修复必须合入；归属校验、真实计费优先级与充值后 Play 联动必须保留。
- 验证：usage billing unit/integration tests、model pricing tests、payment lifecycle tests；线上以测试订单检查余额到账和 boost 状态。
