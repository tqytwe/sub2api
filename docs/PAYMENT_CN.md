...
余额充值订单会区分支付侧金额和用户余额到账金额：

| 字段 / 概念 | 说明 |
|-------------|------|
| `pay_amount` | 发给支付网关的实付金额，含支付手续费；网关验签、每日限额和通道限额仍按支付侧金额校验 |
| 输入金额 | 用户在充值页输入的充值金额，不含手续费 |
| 基础到账 | `input_amount * payment_balance_recharge_multiplier`，四舍五入到 2 位 |
| VIP 加赠 | 创建订单时的当前 VIP 充值加赠；服务端钳制到 `0-10%` |
| 活动加赠 | 当前 Play 活动充值加赠；多活动同时生效时取最高活动加赠 |
| `amount` | 最终到账余额：`base_credited * (1 + vip_bonus_pct/100 + campaign_bonus_pct/100)`，四舍五入到 2 位 |

VIP 不改变 API 计费公式，不降低网关实付金额，也不改变订阅订单价格；VIP 只影响余额充值成功后的到账加赠。本单按充值前 VIP 等级计算，如果这笔充值完成后升级，新等级从下一笔充值订单开始生效。

每笔余额充值订单都会写入 `payment_orders.recharge_snapshot`，固化创建订单时的计算依据：输入金额、基础到账、基础倍率、当前 VIP、VIP 加赠、活动加赠、活动 ID、最终到账、充值前累计，以及是否下一单才享受升级。履约和订单详情解释历史订单时必须以快照为准，不能用当前设置反推。

`users.total_recharged` 只增加 `base_credited`。VIP 加赠、活动加赠、签到、兑换码、管理员加款都不增加 VIP 进度。余额充值订单的邀请返利基数也使用 `base_credited`，避免 VIP 加赠继续放大返利。

退款成功时，用户余额按本单最终到账金额扣回；VIP 累计按订单快照中的基础到账金额回退，部分退款按比例回退。旧的无快照订单会回退到按订单存储金额兼容处理。
...
用户选择充值金额和支付方式
  ...
  支付回调验签 → 订单 PAID
       │
       ▼
  自动发放最终到账余额（RECHARGING）
  ├─ total_recharged 只增加基础到账金额
  └─ 订单 COMPLETED
```
