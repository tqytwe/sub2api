...
6. `artifact_mode` 必须声明为 `browser-capture` 或 `static-review-board`。静态审查板只能作为无浏览器环境下的辅助证据，必须在 residual risk 中写明仍需浏览器截图或最终验收；不得把静态图冒充浏览器截图。
7. PNG 产物必须是真实可解码图片，门禁会校验 PNG chunk、CRC、像素数据和最小尺寸，1x1 或伪造 header 不能通过。
8. 不得提交模板占位符、凭据、Cookie、用户隐私或生产密钥。

`pnpm design:check` 会验证结构化字段、文件覆盖、至少两个视口、键盘与
reduced-motion 结论、产物来源模式，以及修改前后产物确实存在且可解码。纯标题或空壳记录不能通过。
