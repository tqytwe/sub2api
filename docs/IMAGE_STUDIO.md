...
开发者一次多张使用同步 Images 的 `n=1-10`；单个长请求使用 Gateway async；多个
prompt、可恢复、可取消和可下载的持久任务使用 [Batch Image 持久批任务](./BATCH_IMAGE_API.md)。
[Batch Image MVP](./BATCH_IMAGE_MVP.md) 只保留 provider 和内部生命周期参考，不得把
Gateway async、Batch Image 与图像工作室三套任务或计费链路合并。
