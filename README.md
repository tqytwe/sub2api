...
Long-running OpenAI-compatible image generation and editing can be submitted through `/v1/images/generations/async` or `/v1/images/edits/async`, then polled at `/v1/images/tasks/{task_id}` without holding a CDN connection open. Production requires `IMAGE_STORAGE_ENABLED=true`, `IMAGE_ASYNC_QUEUE_ENABLED=true`, `IMAGE_ASYNC_ENABLED=true`, and a configured worker count such as `IMAGE_ASYNC_WORKER_COUNT=4`; the default local backend stores completed images on the persisted data volume and serves authenticated `/v1/images/task-assets/...` URLs. Jisudeng production uses RustFS/S3-compatible storage at `https://jisu.zeabur.app`, bucket `image-task-results`, prefix `images/`, and `IMAGE_STORAGE_PRESIGN_EXPIRY_HOURS=24`; S3/R2 is optional for multi-replica or CDN deployments. See [Asynchronous Image Tasks](docs/ASYNC_IMAGE_TASKS.md) for setup, request, response, and verification examples.
