...
BEGIN
    ...
    monitor_constraint_def := lower(coalesce(monitor_constraint_def, ''));
    IF monitor_constraint_def = ''
    ...
    template_constraint_def := lower(coalesce(template_constraint_def, ''));
    IF template_constraint_def = ''
...
-- PostgreSQL has no ADD CONSTRAINT IF NOT EXISTS. Recreate this named
-- constraint so rerunning a partially applied migration cannot fail.
ALTER TABLE channel_monitors
