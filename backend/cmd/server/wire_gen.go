...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	balanceLedgerService := service.NewBalanceLedgerService(db, apiKeyAuthCacheInvalidator, billingCacheService)
	promoService := service.NewPromoService(promoCodeRepository, userRepository, billingCacheService, client, apiKeyAuthCacheInvalidator, balanceLedgerService)
	dailyCardEntitlementRepository := repository.NewDailyCardEntitlementRepository(client)
	dailyCardService := service.NewDailyCardService(dailyCardEntitlementRepository)
	subscriptionService := service.ProvideSubscriptionService(groupRepository, userSubscriptionRepository, billingCacheService, client, configConfig, dailyCardService)
	affiliateRepository := repository.NewAffiliateRepository(client, db)
	affiliateService := service.ProvideAffiliateService(affiliateRepository, settingService, apiKeyAuthCacheInvalidator, billingCacheService, balanceLedgerService)
	ipRiskRepository := repository.NewIPRiskRepository(db)
	leaderLockCache := repository.NewLeaderLockCache(redisClient)
	ipRiskHasher := service.ProvideIPRiskHasher(configConfig)
	ipRiskRuntimeConfig := service.ProvideIPRiskRuntimeConfig(configConfig)
	notificationEmailService := service.NewNotificationEmailService(settingRepository, emailService)
	ipRiskAlertEmailNotifier := service.NewIPRiskAlertEmailNotifier(settingRepository, notificationEmailService)
	...
	usageService := service.ProvideUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator, balanceLedgerService)
	opsRepository := repository.NewOpsRepository(db)
	usageBillingRepository := repository.NewUsageBillingRepositoryWithLedger(client, db, balanceLedgerService)
	...
	modelPricingResolver := service.NewModelPricingResolverWithCatalog(channelService, billingService, modelCatalogRepository)
	...
	announcementAssetService := service.ProvideAnnouncementAssetService(imageStorageSettingService)
	announcementHandler := handler.ProvideAnnouncementHandler(announcementService, announcementAssetService)
	...
	adminAnnouncementHandler := handler.ProvideAdminAnnouncementHandler(announcementService, announcementAssetService)
	...
	couponHandler := admin.NewCouponHandler(couponService)
	...
	playRepository := repository.NewPlayRepository(client, db)
	mobilePushConfig := repository.ProvideMobilePushConfig()
	mobilePushRepository := repository.ProvideMobilePushRepository(db, mobilePushConfig)
	fcmSender, err := repository.ProvideMobilePushSender(mobilePushConfig, configConfig)
	if err != nil {
		return nil, err
	}
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, playService, balanceLedgerService, couponService, dailyCardService)
	...
	imageTaskService := service.ProvideImageTaskService(imageTaskStore, imageTaskQueue, imageStorageSettingService, secretEncryptor, imageTaskRuntimeState, configConfig)
	...
	fundManagementService := service.NewFundManagementService(db, balanceLedgerService, withdrawalService)
	fundHandler := admin.NewFundHandler(fundManagementService)
	...
	playHandler := handler.ProvidePlayHandler(playService, billingService, announcementAssetService)
