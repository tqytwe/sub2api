...
func (s *PaymentService) doBalance(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease) error {
	...
	// Payment fulfillment is a trusted server-side path. It must not be blocked
	// by the public user's failed-redeem limiter, while retaining the existing
	// ledger, affiliate and membership-projection context.
