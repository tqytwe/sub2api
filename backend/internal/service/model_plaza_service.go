...
func (s *ModelPlazaService) ListGroups(ctx context.Context) ([]PlazaGroup, error) {
	return s.listGroups(ctx, false)
}

// ListGroupsIncludingEmpty keeps active groups without channel-advertised models
// so the handler can attach explicitly catalog-managed models to them.
func (s *ModelPlazaService) ListGroupsIncludingEmpty(ctx context.Context) ([]PlazaGroup, error) {
	return s.listGroups(ctx, true)
}

func (s *ModelPlazaService) listGroups(ctx context.Context, includeEmpty bool) ([]PlazaGroup, error) {
	...
	for _, gid := range order {
		pg := byGroup[gid]
		if len(pg.Models) == 0 && !includeEmpty {
			continue
		}
