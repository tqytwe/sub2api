...
func TestApplyFirstTokenTierKeepsBaseForMultiplierOnlyInterval(t *testing.T) {
	base := &ModelPricing{
		InputPricePerToken:         5,
		OutputPricePerToken:        30,
		CacheCreationPricePerToken: 6.25,
	...
	(&ModelPricingResolver{}).applyFirstTokenTier(resolved, nil)

	require.Empty(t, resolved.Intervals)
	require.InDelta(t, 10, resolved.BasePricing.InputPricePerToken, 1e-12)
	require.InDelta(t, 45, resolved.BasePricing.OutputPricePerToken, 1e-12)
