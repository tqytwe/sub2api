...
func TestOpenAIGatewayServiceRecordUsage_ImageEditAddsTextAndImageInputTokenCost(t *testing.T) {
	groupID := int64(1203)
	imagePrice := 0.02
	inputPrice := 5e-6
	...
	svc.resolver = NewModelPricingResolver(
		channelService,
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "resp_image_edit_input_tokens",
			Model:     "gpt-image-2",
			Usage: OpenAIUsage{
				InputTokens:       371,
	...
	require.NoError(t, err)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, 352, usageRepo.lastLog.ImageInputTokens)
	require.InDelta(t, float64(19)*inputPrice, usageRepo.lastLog.InputCost, 1e-15)
	require.InDelta(t, float64(352)*imageInputPrice, usageRepo.lastLog.ImageInputCost, 1e-15)
	require.Zero(t, usageRepo.lastLog.OutputCost)
	require.Zero(t, usageRepo.lastLog.ImageOutputCost)
	require.InDelta(t, imagePrice+float64(19)*inputPrice+float64(352)*imageInputPrice, usageRepo.lastLog.TotalCost, 1e-12)
...
func TestOpenAIGatewayServiceRecordUsage_ChannelVideoPricingBillsPerSecond(t *testing.T) {
	groupID := int64(1262)
	perSecondPrice := 0.8
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	svc := newOpenAIRecordUsageServiceForTest(usageRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)
	svc.resolver = newOpenAIVideoChannelPricingResolverForTest(t, groupID, "agnes-video-v2.0", perSecondPrice)

	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:            "agnes-video-channel-per-second",
			ResponseID:           "video-channel-per-second",
			Model:                "agnes-video-v2.0",
			BillingModel:         "agnes-video-v2.0",
			VideoCount:           1,
			VideoResolution:      VideoBillingResolution720P,
		...
		User:    &User{ID: 201262},
		Account: &Account{ID: 301262, Platform: PlatformOpenAI},
	})
...
func newOpenAIVideoChannelPricingResolverForTest(t *testing.T, groupID int64, model string, perSecondPrice float64) *ModelPricingResolver {
	t.Helper()
	cache := newEmptyChannelCache()
	cache.pricingByGroupModel[channelModelKey{groupID: groupID, model: model}] = &ChannelModelPricing{
