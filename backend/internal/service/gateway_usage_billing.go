...
func notifyBalanceLow(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	billedCost := p.billedCost()
	if p.IsSubscriptionBill || billedCost <= 0 || p.User == nil || deps.balanceNotifyService == nil {
	...
	deps.balanceNotifyService.CheckBalanceAfterDeduction(context.Background(), p.User, oldBalance, billedCost)
}
...
func resolveOldBalance(p *postUsageBillingParams, result *UsageBillingApplyResult) float64 {
	if result != nil && result.NewBalance != nil {
		return *result.NewBalance + p.billedCost()
	}
...
// recordUsageOpts preserves the long-context extension point used by older
// callers while allowing the v0.2.0 core path to use the unified billing policy.
