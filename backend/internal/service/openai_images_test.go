...
func TestOpenAIGatewayServiceForwardImages_OAuth429PreservesRetryAfterCooldown(t *testing.T) {
	...
	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)
	// An explicit Retry-After must not be spent retrying this same account.
...
func TestOpenAIImagesOAuthBodyReadTransportErrorFailover(t *testing.T) {
	...
	resp := &http.Response{
		...
		Body: errReadCloser{err: errors.New("stream error: stream ID 11; INTERNAL_ERROR; received from peer")},
	}
	account := &Account{ID: 5400, Name: "openai-oauth", Platform: PlatformOpenAI, Type: AccountTypeOAuth}
	svc := &OpenAIGatewayService{}

	_, _, _, readErr := svc.handleOpenAIImagesOAuthNonStreamingResponse(resp, c, &OpenAIImagesRequest{ResponseFormat: "b64_json"}, "gpt-image-2")
...
func TestOpenAIGatewayServiceForwardImages_OAuthFansOutTenImages(t *testing.T) {
	...
	result, err := svc.ForwardImages(withOpenAIImagesForceResponses(context.Background()), c, &Account{
		ID:       3,
		Name:     "openai-oauth",
		Platform: PlatformOpenAI,
		Type:     AccountTypeOAuth,
