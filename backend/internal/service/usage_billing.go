...
//
//	余额存储值减去 amount            // 存剩余额度，舍入的是「减法结果」
//	quota_used 累加 amount           // 存累计用量，舍入的是「加法结果」
//
...
func buildUsageBillingFingerprint(c *UsageBillingCommand) string {
	...
	raw := fmt.Sprintf(
		"%d|%d|%d|%s|%s|%s|%s|%d|%d|%d|%d|%d|%d|%s|%d|%0.10f|%0.10f|%s|%0.10f|%0.10f|%0.10f|%0.10f|%0.10f|%0.10f",
		c.UserID,
		c.AccountID,
		c.APIKeyID,
		...
		c.ActualCost,
		c.BillingSurchargeCost,
		strings.TrimSpace(c.BillingSurchargeMode),
		c.BillingSurchargeValue,
		c.BalanceCost,
		c.SubscriptionCost,
		c.APIKeyQuotaCost,
