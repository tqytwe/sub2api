...
func ParseGrokMediaRequest(contentType string, body []byte) GrokMediaRequestInfo {
	info, _ := ParseGrokMediaRequestWithError(contentType, body)
	return info
}

func ParseGrokMediaRequestWithError(contentType string, body []byte) (GrokMediaRequestInfo, error) {
	...
	if info.ImageResolution != "" {
		info.SizeTier = strings.ToUpper(info.ImageResolution)
	}
...
func ExtractGrokVideoBillingFromStatusBody(statusBody []byte, pending *GrokVideoPendingBilling, requestID string) *OpenAIForwardResult {
	...
	return &OpenAIForwardResult{ResponseID: responseID, Model: model, BillingModel: billingModel, UpstreamModel: upstreamModel, VideoCount: 1, VideoResolution: resolution, VideoDurationSeconds: duration}
}
...
func (s *OpenAIGatewayService) SelectMediaVideoRequestAccount(ctx context.Context, groupID *int64, sessionHash string, accountID int64, requestedModel, platform string) (*AccountSelectionResult, OpenAIAccountScheduleDecision, error) {
	decision := OpenAIAccountScheduleDecision{Layer: openAIAccountScheduleLayerSessionSticky}
	if accountID <= 0 || strings.TrimSpace(sessionHash) == "" { return nil, decision, ErrNoAvailableAccounts }
	...
	selection, _, err := scheduler.selectBySessionHash(ctx, OpenAIAccountScheduleRequest{GroupID: groupID, Platform: platform, SessionHash: sessionHash, StickyAccountID: accountID, PreserveStickyBinding: true, DisableStickyEscape: true, RequestedModel: requestedModel, RequiredTransport: OpenAIUpstreamTransportHTTPSSE, RequirePrivacySet: s.openAIGroupRequiresPrivacySet(ctx, groupID)})
	if err != nil { return nil, decision, err }
	if selection == nil || selection.Account == nil { return nil, decision, ErrNoAvailableAccounts }
...
func (s *OpenAIGatewayService) ForwardGrokMedia(
	...
	requestInfo, err := ParseGrokMediaRequestWithError(contentType, body)
	if err != nil {
		return nil, err
	}
...
func prepareGrokMediaForwardBody(endpoint GrokMediaEndpoint, body []byte, contentType string) ([]byte, string, error) {
	...
	info, err := ParseGrokMediaRequestWithError(contentType, body)
	if err != nil {
		return nil, "", err
	}
...
func grokMediaUsageFromResponse(endpoint GrokMediaEndpoint, requestInfo GrokMediaRequestInfo, responseBody []byte) grokMediaUsageMetadata {
	...
	case GrokMediaEndpointVideoStatus:
		// Provisional units are merged with the stored create-time snapshot before
		// RecordUsage, so a completed async video is billed exactly once.
