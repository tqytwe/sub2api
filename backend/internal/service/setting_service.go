...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	if settings.PlayCheckinDailyReward > 0 {
		updates[SettingKeyPlayCheckinDailyReward] = strconv.FormatFloat(settings.PlayCheckinDailyReward, 'f', 8, 64)
	}
	...
	if settings.PlayBlindboxCost > 0 {
		updates[SettingKeyPlayBlindboxCost] = strconv.FormatFloat(settings.PlayBlindboxCost, 'f', 8, 64)
	}
	if settings.PlayBlindboxDailyLimit > 0 {
		updates[SettingKeyPlayBlindboxDailyLimit] = strconv.Itoa(settings.PlayBlindboxDailyLimit)
	}
	updates[SettingKeyPlayQuizEnabled] = strconv.FormatBool(settings.PlayQuizEnabled)
	if settings.PlayQuizRewardPerCorrect > 0 {
		updates[SettingKeyPlayQuizRewardPerCorrect] = strconv.FormatFloat(settings.PlayQuizRewardPerCorrect, 'f', 8, 64)
	}
	if settings.PlayQuizQuestionsPerDay > 0 {
		updates[SettingKeyPlayQuizQuestionsPerDay] = strconv.Itoa(settings.PlayQuizQuestionsPerDay)
	}
...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		// Play features (default disabled; opt-in)
		...
		SettingKeyPlayBlindboxCost:         "0.5",
		...
		SettingKeyPublicModelsEnabled:      "false",
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	if v, err := strconv.ParseFloat(settings[SettingKeyPlayCheckinDailyReward], 64); err == nil && v > 0 {
		result.PlayCheckinDailyReward = v
	} else {
		result.PlayCheckinDailyReward = 0.5
	}
	...
	if v, err := strconv.ParseFloat(settings[SettingKeyPlayBlindboxCost], 64); err == nil && v > 0 {
		result.PlayBlindboxCost = v
	} else {
		result.PlayBlindboxCost = 0.5
	}
	if v, err := strconv.Atoi(settings[SettingKeyPlayBlindboxDailyLimit]); err == nil && v > 0 {
		result.PlayBlindboxDailyLimit = v
	} else {
		result.PlayBlindboxDailyLimit = 10
	}
	result.PlayQuizEnabled = settings[SettingKeyPlayQuizEnabled] == "true"
	if v, err := strconv.ParseFloat(settings[SettingKeyPlayQuizRewardPerCorrect], 64); err == nil && v > 0 {
		result.PlayQuizRewardPerCorrect = v
	} else {
		result.PlayQuizRewardPerCorrect = 0.1
	}
	if v, err := strconv.Atoi(settings[SettingKeyPlayQuizQuestionsPerDay]); err == nil && v > 0 {
		result.PlayQuizQuestionsPerDay = v
	} else {
		result.PlayQuizQuestionsPerDay = 5
	}
