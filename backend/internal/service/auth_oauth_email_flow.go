...
func (s *AuthService) RollbackOAuthEmailAccountCreation(ctx context.Context, userID int64, invitationCode string) error {
	...
	if rollbackRepo, ok := s.userRepo.(interface {
		DeleteOAuthEmailAccountCreation(context.Context, int64) error
	}); ok {
		if err := rollbackRepo.DeleteOAuthEmailAccountCreation(ctx, userID); err != nil {
