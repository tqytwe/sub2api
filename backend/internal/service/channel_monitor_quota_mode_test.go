...
func TestRunCheck_QuotaModeRemainsAvailableInV2(t *testing.T) {
	repo := &quotaModeRepoStub{monitor: &ChannelMonitor{
		ID:              6,
		Name:            "antigravity-quota-v2",
		Provider:        MonitorProviderAntigravity,
		PrimaryModel:    MonitorDefaultQuotaModel,
		Enabled:         true,
		IntervalSeconds: 60,
		CheckMode:       MonitorCheckModeQuota,
		AccountID:       int64Ptr(16),
	}}
	svc := newQuotaModeService(repo)
	svc.SetRuntimeReader(channelMonitorRuntimeStub{rt: ChannelMonitorRuntime{
		Enabled: true,
	...
	fetcher := newQuotaModeFetcher(map[int64]*Account{
		16: {ID: 16, Platform: domain.PlatformAntigravity},
	}, &stubMonitorUsageSource{usage: &UsageInfo{
...
func TestRunCheck_QuotaProbeRemainsRetiredInV2(t *testing.T) {
	repo := &quotaModeRepoStub{monitor: &ChannelMonitor{
		ID:              7,
	...
	_, err := svc.RunCheck(context.Background(), repo.monitor.ID)
