...
type CaptchaProof struct {
	// TurnstileToken carries a Cloudflare token or Aliyun captchaVerifyParam.
...
// VerifyCaptchaForRegister skips duplicate validation after the verified email-code step.
...
func (s *AuthService) VerifyActionCaptchaIfEnabled(ctx context.Context, proof CaptchaProof, remoteIP string) error {
	...
	if !providerConfig.Tencent.Enabled && !providerConfig.Aliyun.Enabled {
		return nil
	}
...
// Legacy internal helpers remain for callers outside the HTTP handler.
