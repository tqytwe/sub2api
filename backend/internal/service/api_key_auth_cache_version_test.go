...
func TestAPIKeyService_RejectsV16AuthSnapshotWithMissingBillingSurchargeProjection(t *testing.T) {
	groupID := int64(9)
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-v16-missing-surcharge", &APIKeyAuthCacheEntry{
		Snapshot: &APIKeyAuthSnapshot{
			Version:  16,
			APIKeyID: 1,
			UserID:   2,
	...
	if ok {
		t.Fatal("expected v16 auth snapshot to be rejected after fixing the billing surcharge DB projection")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
...
func TestAPIKeyService_RejectsV17AuthSnapshotWithoutCombinedForkAndLiveProjection(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-v17-partial-projection", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v17 auth snapshot to be rejected after combining surcharge and Live fields")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
