...
func TestListGroups_DisplayPricingPreservesCacheWrite1hPrice(t *testing.T) {
	channels := []Channel{{
		ID: 1, Name: "ch", Status: StatusActive, GroupIDs: []int64{10},
		ModelPricing: []ChannelModelPricing{{
	...
	require.NotNil(t, pricing.CacheWritePrice)
	require.InDelta(t, 4e-6, *pricing.CacheWritePrice, 1e-15)
	require.NotNil(t, pricing.CacheWrite1hPrice)
	require.InDelta(t, 7e-6, *pricing.CacheWrite1hPrice, 1e-15)
}
