...
func (s *ChannelMonitorService) RunCheck(ctx context.Context, id int64) ([]*CheckResult, error) {
	...
	// A missing repository is only expected in isolated unit tests or a broken
	// wiring; fail closed before dereferencing it. V2 must never probe upstream.
	if s == nil || s.repo == nil {
	...
	checkMode := defaultCheckMode(m.CheckMode)
	if !rt.AllowsCheckMode(checkMode) {
		return nil, ErrChannelMonitorActiveProbesRetired
	}
