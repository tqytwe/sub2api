...
//
// Unlike ApplyDelta this does NOT run after-commit side effects: it returns while
// the caller's transaction is still open, so callers must invalidate balance
// caches themselves after their commit.
