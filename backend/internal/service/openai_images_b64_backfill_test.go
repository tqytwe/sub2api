...
func TestOpenAIGatewayServiceForwardImages_APIKeyLeavesURLOnlyResponseWhenDisabled(t *testing.T) {
	...
	require.Len(t, upstream.requests, 1)
	require.Equal(t, http.StatusOK, rec.Code)
	// The account opt-out prevents only the additional URL download and b64_json
	// injection. The Fork's normal image response decoration remains intact.
	require.Equal(t, "https://cdn.example.com/cat.png", gjson.Get(rec.Body.String(), "data.0.url").String())
	require.False(t, gjson.Get(rec.Body.String(), "data.0.b64_json").Exists())
	require.Equal(t, int64(1710000000), gjson.Get(rec.Body.String(), "created").Int())
	require.Equal(t, int64(1), gjson.Get(rec.Body.String(), "requested_n").Int())
