...
// newOptionalJWTMiddlewareTestEnv 创建 OptionalJWT 中间件测试环境。
// handler 回写「是否携带 AuthSubject」,便于断言匿名 vs 登录两种路径。
func newOptionalJWTMiddlewareTestEnv(users map[int64]*service.User) (*gin.Engine, *service.AuthService) {
...
func TestOptionalJWTAuth_NoHeaderPassesAnonymously(t *testing.T) {
	router, _ := newOptionalJWTMiddlewareTestEnv(nil)
...
func TestOptionalJWTAuth_ValidTokenSetsSubject(t *testing.T) {
	...
	router, authSvc := newOptionalJWTMiddlewareTestEnv(map[int64]*service.User{7: user})
