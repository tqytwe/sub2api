...
func (c *Config) Validate() error {
	...
	if c.BatchImage.Enabled && !c.BatchImage.QueueEnabled {
		return fmt.Errorf("batch_image.enabled requires batch_image.queue_enabled")
	}
...
func ValidateAbsoluteHTTPURL(raw string) error {
	...
	// url.URL cannot distinguish an absent fragment from a trailing empty '#'.
	// Reject the raw separator as well, otherwise a configured base URL such as
	// https://www.jisudeng.com/# swallows every appended reset/redirect path.
