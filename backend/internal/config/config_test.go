...
func TestLoadImageStorageCleanupDefaultsAndOverrides(t *testing.T) {
	resetViperWithJWTSecret(t)
	cfg, err := Load()
	require.NoError(t, err)
	require.Equal(t, 60, cfg.ImageStorage.CleanupIntervalSeconds)
	require.Equal(t, 100, cfg.ImageStorage.CleanupBatchSize)

	resetViperWithJWTSecret(t)
	t.Setenv("IMAGE_STORAGE_CLEANUP_INTERVAL_SECONDS", "15")
	t.Setenv("IMAGE_STORAGE_CLEANUP_BATCH_SIZE", "25")
	cfg, err = Load()
	require.NoError(t, err)
	require.Equal(t, 15, cfg.ImageStorage.CleanupIntervalSeconds)
...
func TestLoadAsyncImageRejectsAPIEnabledWithoutQueue(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("IMAGE_STORAGE_ENABLED", "true")
	t.Setenv("IMAGE_ASYNC_ENABLED", "true")
	t.Setenv("IMAGE_ASYNC_QUEUE_ENABLED", "false")

	_, err := Load()

	require.ErrorContains(t, err, "image_async.enabled requires image_async.queue_enabled")
}

func TestLoadAsyncImageRejectsQueueWithoutStorage(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("IMAGE_ASYNC_ENABLED", "false")
	t.Setenv("IMAGE_ASYNC_QUEUE_ENABLED", "true")

	_, err := Load()

	require.ErrorContains(t, err, "image_async.queue_enabled requires active image_storage")
}

func TestLoadAsyncImageAllowsQueueDrainWithAPIDisabled(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("IMAGE_STORAGE_ENABLED", "true")
	t.Setenv("IMAGE_ASYNC_ENABLED", "false")
	t.Setenv("IMAGE_ASYNC_QUEUE_ENABLED", "true")
	t.Setenv("IMAGE_ASYNC_WORKER_COUNT", "3")
	t.Setenv("TOTP_ENCRYPTION_KEY", strings.Repeat("ab", 32))

	cfg, err := Load()

	require.NoError(t, err)
	require.False(t, cfg.ImageAsync.Enabled)
	require.True(t, cfg.ImageAsync.QueueEnabled)
	require.Equal(t, 3, cfg.ImageAsync.WorkerCount)
}

func TestLoadAsyncImageRejectsAutoGeneratedEncryptionKey(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("IMAGE_STORAGE_ENABLED", "true")
	t.Setenv("IMAGE_ASYNC_ENABLED", "false")
	t.Setenv("IMAGE_ASYNC_QUEUE_ENABLED", "true")
	...
	require.ErrorContains(t, err, "image_async.queue_enabled requires a fixed totp.encryption_key")
}

func TestLoadAsyncImageRejectsHeartbeatAtOrBeyondJobLockTTL(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("IMAGE_STORAGE_ENABLED", "true")
	t.Setenv("IMAGE_ASYNC_ENABLED", "false")
	t.Setenv("IMAGE_ASYNC_QUEUE_ENABLED", "true")
	t.Setenv("IMAGE_ASYNC_JOB_LOCK_TTL_SECONDS", "30")
	t.Setenv("IMAGE_ASYNC_HEARTBEAT_SECONDS", "30")
	t.Setenv("TOTP_ENCRYPTION_KEY", strings.Repeat("ab", 32))

	_, err := Load()

	require.ErrorContains(t, err, "image_async.heartbeat_seconds must be less than image_async.job_lock_ttl_seconds")
}
...
func TestBatchImageDeploymentTemplatesExposeRuntimeFlags(t *testing.T) {
	...
	require.Contains(t, zeabur, "IMAGE_ASYNC_QUEUE_ENABLED:\n            default: \"true\"")
	require.Contains(t, zeabur, "IMAGE_ASYNC_ENABLED:\n            default: \"true\"")
	require.Contains(t, zeabur, "persistent /data")
	require.NotContains(t, zeabur, "persistent /app/data")
	require.Contains(t, zeabur, "dir: /data")
	require.Contains(t, zeabur, "--appendonly yes --appendfsync everysec")
	...
	for _, path := range []string{"deploy/docker-compose.yml", "deploy/docker-compose.server.yml"} {
		...
		require.Contains(t, compose, "IMAGE_ASYNC_QUEUE_ENABLED=${IMAGE_ASYNC_QUEUE_ENABLED:-false}")
		require.Contains(t, compose, "IMAGE_ASYNC_ENABLED=${IMAGE_ASYNC_ENABLED:-false}")
	}

	localCompose := read("deploy/docker-compose.local.yml")
	require.Contains(t, localCompose, "BATCH_IMAGE_QUEUE_ENABLED=${BATCH_IMAGE_QUEUE_ENABLED:-false}")
	require.Contains(t, localCompose, "BATCH_IMAGE_ENABLED=${BATCH_IMAGE_ENABLED:-false}")
	require.Contains(t, localCompose, "IMAGE_ASYNC_QUEUE_ENABLED=${IMAGE_ASYNC_QUEUE_ENABLED:-false}")
	require.Contains(t, localCompose, "IMAGE_ASYNC_ENABLED=${IMAGE_ASYNC_ENABLED:-false}")
	...
	require.Contains(t, envExample, "IMAGE_ASYNC_QUEUE_ENABLED=false")
	require.Contains(t, envExample, "IMAGE_ASYNC_ENABLED=false")
	...
	require.Contains(t, configExample, "image_async:")
}
