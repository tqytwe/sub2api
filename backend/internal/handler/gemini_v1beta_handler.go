...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	if billingEligibilityErr != nil {
		reqLog.Info("gemini.billing_eligibility_check_failed", zap.Error(billingEligibilityErr))
		status, _, message, retryAfter := billingErrorDetails(billingEligibilityErr)
		if retryAfter > 0 {
