...
func TestToModelPlazaGroupDTO_LongContextTiersAndBasis(t *testing.T) {
	...
	g := service.PlazaGroup{
		...
		Models: []service.PlazaModel{{
			...
			Pricing: &service.ChannelModelPricing{
				...
				CacheWrite1hPrice: testPtr(6e-6),
				Intervals: []service.PricingInterval{
					{MinTokens: 0, MaxTokens: &maxTokens, TierLabel: "≤272K", InputPrice: testPtr(2.5e-6), CacheWrite1hPrice: testPtr(6e-6)},
					{MinTokens: 272000, TierLabel: ">272K", InputPrice: testPtr(5e-6), CacheWrite1hPrice: testPtr(12e-6)},
				},
			},
			OfficialPricing: &service.PlazaOfficialPricing{
				InputPrice:        testPtr(2.5e-6),
				CacheWrite1hPrice: testPtr(7e-6),
				Intervals: []service.PricingInterval{
					{MinTokens: 0, MaxTokens: &maxTokens, TierLabel: "≤272K", InputPrice: testPtr(2.5e-6), CacheWrite1hPrice: testPtr(7e-6)},
					{MinTokens: 272000, TierLabel: ">272K", InputPrice: testPtr(5e-6), CacheWrite1hPrice: testPtr(14e-6)},
				},
			},
	...
	pricing := model["display_pricing"].(map[string]any)
	require.InDelta(t, 6e-6, pricing["cache_write_1h_price"].(float64), 1e-15)
	paidTiers := pricing["intervals"].([]any)
	require.Len(t, paidTiers, 2)
	paidFirst := paidTiers[0].(map[string]any)
	paidSecond := paidTiers[1].(map[string]any)
	require.InDelta(t, 6e-6, paidFirst["cache_write_1h_price"].(float64), 1e-15)
	require.Equal(t, ">272K", paidSecond["tier_label"])
	require.InDelta(t, 12e-6, paidSecond["cache_write_1h_price"].(float64), 1e-15)

	official := model["official_pricing"].(map[string]any)
	require.InDelta(t, 7e-6, official["cache_write_1h_price"].(float64), 1e-15)
	officialTiers := official["intervals"].([]any)
	require.Len(t, officialTiers, 2)
	first := officialTiers[0].(map[string]any)
	second := officialTiers[1].(map[string]any)
	...
	require.InDelta(t, 7e-6, first["cache_write_1h_price"].(float64), 1e-15)
	require.InDelta(t, 14e-6, second["cache_write_1h_price"].(float64), 1e-15)
}
