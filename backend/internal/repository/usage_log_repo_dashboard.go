...
func (r *usageLogRepository) GetUserStats(ctx context.Context, userID int64, startTime, endTime time.Time) (*UserStats, error) {
	query := `
		SELECT
			...
			COALESCE(SUM(` + usageLogChargedCostExpr + `), 0) as total_cost,
	...
	combinedStatsQuery := `
		WITH scoped AS (
			SELECT
				...
				cache_read_tokens,
				total_cost,
				` + usageLogChargedCostExpr + ` AS actual_cost,
...
func (r *usageLogRepository) GetUserDashboardStats(ctx context.Context, userID int64) (*UserDashboardStats, error) {
	...
	todayStatsQuery := `
		SELECT
			...
			COALESCE(SUM(` + usageLogChargedCostExpr + `), 0) as today_actual_cost
		FROM usage_logs
	...
	platformQuery := `
		SELECT
			...
			COALESCE(SUM(` + usageLogChargedCostExprUL + `), 0) as total_actual_cost,
			...
			COALESCE(SUM(` + usageLogChargedCostExprUL + `) FILTER (WHERE ul.created_at >= $2), 0) as today_actual_cost
		FROM usage_logs ul
