...
func (r *usageLogRepository) GetUserUsageTrend(ctx context.Context, startTime, endTime time.Time, granularity string, limit int) (results []UserUsageTrendPoint, err error) {
	...
	query := fmt.Sprintf(`
		...
		SELECT
			...
			COALESCE(SUM(`+usageLogChargedCostExprU+`), 0) as actual_cost
		FROM usage_logs u
...
func (r *usageLogRepository) GetUserUsageTrendByUserID(ctx context.Context, userID int64, startTime, endTime time.Time, granularity string) (results []TrendDataPoint, err error) {
	...
	query := fmt.Sprintf(`
		SELECT
			...
			COALESCE(SUM(`+usageLogChargedCostExpr+`), 0) as actual_cost
		FROM usage_logs
...
func (r *usageLogRepository) getGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, billingMode string) (results []usagestats.GroupStat, err error) {
	query := `
		SELECT
			...
			COALESCE(SUM(` + usageLogChargedCostExprUL + `), 0) as actual_cost,
...
func (r *usageLogRepository) GetAllGroupUsageSummary(ctx context.Context, todayStart time.Time) ([]usagestats.GroupUsageSummary, error) {
	query := `
		SELECT
			g.id AS group_id,
			COALESCE(SUM(` + usageLogChargedCostExprUL + `), 0) AS total_cost,
			COALESCE(SUM(CASE WHEN ul.created_at >= $1 THEN ` + usageLogChargedCostExprUL + ` ELSE 0 END), 0) AS today_cost
		FROM groups g
