  ...
  >
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <span v-if="form.provider_key === 'bepusdt'" class="text-xs text-gray-500 dark:text-gray-400">
          {{ t('admin.settings.payment.bepusdtManualRefundHint') }}
        </span>
...
const supportsRefunds = computed(() => form.provider_key !== 'bepusdt')
...
function handleSave() {
  ...
  emit('save', {
    ...
    refund_enabled: supportsRefunds.value ? form.refund_enabled : false,
...
function loadProvider(provider: ProviderInstance) {
  ...
  form.refund_enabled = provider.provider_key === 'bepusdt' ? false : provider.refund_enabled
  form.allow_user_refund = provider.provider_key === 'bepusdt' ? false : provider.allow_user_refund
  clearConfig()
