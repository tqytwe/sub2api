    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-gray-100 p-2 dark:bg-dark-800">
          <Icon name="dollar" size="md" class="text-gray-900 dark:text-white" :stroke-width="2" />
        </div>
        <div>
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.balance') }}</p>
          <p class="text-xl font-bold tabular-nums text-gray-900 dark:text-white">${{ formatBalance(balance) }}</p>
