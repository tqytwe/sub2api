...
//
// 条目顺序：密钥 → AI创作空间 → 用量 → 渠道状态 → 订阅/支付 → 玩法福利 → 资料。
// 可用渠道不在侧栏展示；渠道状态是经脱敏的用户只读视图，由渠道监控开关控制。
...
function finalizeNav(items: NavItem[]): NavItem[] {
  ...
  const filtered = authStore.isSimpleMode ? visible.filter(item => !item.hideInSimpleMode) : visible
  ...
  const baseItems: NavItem[] = [
    ...
    },
    {
      ...
      children: [
        ...
        { path: '/admin/orders/play-billing', label: t('nav.playBilling'), icon: CreditCardIcon },
      ],
    },
