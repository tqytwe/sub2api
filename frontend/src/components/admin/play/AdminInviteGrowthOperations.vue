...
function blankCampaignDraft(): CampaignDraft {
  return {
    ...
    public_rules_md: t('admin.playOps.inviteGrowth.defaults.publicRules'),
    invitee_notice_md: t('admin.playOps.inviteGrowth.defaults.inviteeNotice'),
    legacy_rebate_policy: 'exclude',
...
function openEditCampaign(): void {
  ...
  campaignDraft.value = {
    ...
    public_rules_md: campaign.public_rules_md,
    invitee_notice_md: campaign.invitee_notice_md,
    legacy_rebate_policy: campaign.legacy_rebate_policy,
...
function buildCampaignInput(): AdminReferralCampaignInput {
  ...
  if (!draft.public_rules_md.trim() || !draft.invitee_notice_md.trim()) {
    throw new Error(t('admin.playOps.inviteGrowth.errors.publicCopyRequired'))
  }
  ...
  return {
    ...
    public_rules_md: draft.public_rules_md.trim(),
    invitee_notice_md: draft.invitee_notice_md.trim(),
    legacy_rebate_policy: draft.legacy_rebate_policy,
  ...
  <section class="space-y-5" aria-labelledby="invite-growth-title" :aria-busy="loading">
    ...
    <div class="grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(340px,0.6fr)]">
      ...
      <aside class="card p-5" aria-live="polite">
        ...
        <div v-else-if="detail" class="space-y-5">
          <div class="flex items-start justify-between gap-3"><div><h3 class="font-semibold text-gray-900 dark:text-white">{{ detail.campaign.name }}</h3><p class="mt-1 text-xs text-gray-500">{{ t('admin.playOps.inviteGrowth.campaignVersion', { key: detail.campaign.key, version: detail.campaign.version, rulesVersion: detail.campaign.rules_version }) }}</p><p v-if="detail.pending_financial_version" class="mt-2 text-xs text-amber-700 dark:text-amber-300">{{ t('admin.playOps.inviteGrowth.pendingFinancialVersion', { version: detail.pending_financial_version.rules_version, rulesVersion: detail.campaign.rules_version }) }}</p></div><button v-if="!['settling','closed','cancelled'].includes(detail.campaign.status)" type="button" class="btn btn-secondary" @click="openEditCampaign"><Icon name="edit" size="sm" />{{ t('admin.playOps.inviteGrowth.editCampaign') }}</button></div>
    ...
    <BaseDialog :show="showCampaignDialog" :title="campaignDraft.id ? t('admin.playOps.inviteGrowth.editCampaign') : t('admin.playOps.inviteGrowth.newCampaign')" width="extra-wide" @close="showCampaignDialog = false">
      <form class="space-y-5" @submit.prevent="saveCampaign">
        ...
        <fieldset class="space-y-2 rounded border border-amber-200 p-4 dark:border-amber-900"><legend class="px-1 text-sm font-medium">{{ t('admin.playOps.inviteGrowth.legacyRebate.title') }}</legend><p class="text-xs text-gray-500">{{ t('admin.playOps.inviteGrowth.legacyRebate.hint') }}</p><div class="mt-3 flex flex-wrap gap-4"><label class="inline-flex items-center gap-2"><input v-model="campaignDraft.legacy_rebate_policy" type="radio" value="exclude" />{{ t('admin.playOps.inviteGrowth.legacyRebate.exclude') }}</label><label class="inline-flex items-center gap-2"><input v-model="campaignDraft.legacy_rebate_policy" type="radio" value="stack" />{{ t('admin.playOps.inviteGrowth.legacyRebate.stack') }}</label></div></fieldset>
        <div class="grid gap-4 lg:grid-cols-2"><label class="grid gap-1 text-sm"><span>{{ t('admin.playOps.inviteGrowth.publicRulesLabel') }}</span><textarea data-testid="referral-public-rules" v-model="campaignDraft.public_rules_md" rows="8" maxlength="12000" class="input font-mono text-xs" required /></label><label class="grid gap-1 text-sm"><span>{{ t('admin.playOps.inviteGrowth.inviteeNoticeLabel') }}</span><textarea data-testid="referral-invitee-notice" v-model="campaignDraft.invitee_notice_md" rows="8" maxlength="6000" class="input font-mono text-xs" required /></label></div>
