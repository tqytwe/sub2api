...
function blankCampaignDraft(): CampaignDraft {
  return {
    ...
    public_rules_md: '## 活动规则\n\n邀请新用户注册并达到门槛后，按活动档位领取奖励。退款或风控拒绝会影响资格与奖励。',
    invitee_notice_md: '通过本链接注册后，请完成活动要求的净实付与实际消费。',
...
function buildCampaignInput(): AdminReferralCampaignInput {
  ...
  if (!draft.key.trim() || !draft.name.trim()) throw new Error(t('admin.playOps.inviteGrowth.errors.required'))
	if (!draft.public_rules_md.trim() || !draft.invitee_notice_md.trim()) {
		throw new Error('请填写对邀请人与受邀人公开的活动说明。')
	}
  ...
  <section class="space-y-5" aria-labelledby="invite-growth-title" :aria-busy="loading">
    ...
    <BaseDialog :show="showCampaignDialog" :title="campaignDraft.id ? t('admin.playOps.inviteGrowth.editCampaign') : t('admin.playOps.inviteGrowth.newCampaign')" width="extra-wide" @close="showCampaignDialog = false">
      <form class="space-y-5" @submit.prevent="saveCampaign">
        ...
        <fieldset class="space-y-2 rounded border border-amber-200 p-4 dark:border-amber-900"><legend class="px-1 text-sm font-medium">常规 10% 邀请返佣</legend><p class="text-xs text-gray-500">该设置与活动档位发放模式无关，并会写入每个活动邀请的结算快照。</p><div class="mt-3 flex flex-wrap gap-4"><label class="inline-flex items-center gap-2"><input v-model="campaignDraft.legacy_rebate_policy" type="radio" value="exclude" />不叠加（默认）：活动邀请永久不产生常规 10% 返佣</label><label class="inline-flex items-center gap-2"><input v-model="campaignDraft.legacy_rebate_policy" type="radio" value="stack" />允许叠加：活动奖励和常规返佣分别结算</label></div></fieldset>
        <div class="grid gap-4 lg:grid-cols-2"><label class="grid gap-1 text-sm"><span>活动规则（用户报名页可见）</span><textarea v-model="campaignDraft.public_rules_md" rows="8" maxlength="12000" class="input font-mono text-xs" required /></label><label class="grid gap-1 text-sm"><span>受邀人说明（注册链接可见）</span><textarea v-model="campaignDraft.invitee_notice_md" rows="8" maxlength="6000" class="input font-mono text-xs" required /></label></div>
