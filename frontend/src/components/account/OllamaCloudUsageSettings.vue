<template>
  <section v-if="state?.eligible" class="space-y-4 border-t border-gray-200 pt-4 dark:border-dark-600" data-testid="ollama-cloud-usage-settings">
    ...
    <div v-if="loading" class="flex h-20 items-center justify-center text-gray-400">
      ...
      <Icon name="refresh" size="sm" class="animate-spin motion-reduce:animate-none" />
    </div>
    <template v-else>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          @click="refreshUsage"
        >
          <!-- design-governance-allow: continuous-motion - refresh feedback stops when reduced motion is requested. -->
          <Icon name="refresh" size="xs" class="mr-1.5 motion-reduce:animate-none" :class="{ 'animate-spin': refreshing }" />
