  ...
  >
    ...
    <div v-if="state.configured" class="flex items-center pt-0.5">
      <button
        ...
        @click="refreshUsage"
      >
        <!-- design-governance-allow: inline-svg - the refresh mark is a shared small status glyph without a matching Icon path. --> <!-- design-governance-allow: continuous-motion - refresh feedback is user-triggered and disabled for reduced-motion users. -->
        <svg class="h-2.5 w-2.5" :class="{ 'animate-spin motion-reduce:animate-none': refreshing }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
