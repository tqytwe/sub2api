    ...
    datasets: [
      {
        ...
        data: errorRates,
        borderColor: '#ef4444', // design-governance-allow: raw-color - Chart.js canvas requires fixed severity colors.
        backgroundColor: 'rgba(239, 67, 67, 0.10)', // design-governance-allow: raw-color - Chart.js canvas requires fixed severity colors.
        yAxisID: 'yPct',
        tension: 0.4,
      ...
      },
      {
        ...
        data: cacheRates,
        borderColor: '#10b981', // design-governance-allow: raw-color - Chart.js canvas requires fixed status colors.
        backgroundColor: 'rgba(16, 185, 129, 0.08)', // design-governance-allow: raw-color - Chart.js canvas requires fixed status colors.
        yAxisID: 'yPct',
        tension: 0.4,
        cubicInterpolationMode: 'monotone' as const,
        fill: false,
      ...
      },
      {
        ...
        data: ttftP50,
        borderColor: '#0ea5e9', // design-governance-allow: raw-color - Chart.js canvas requires fixed latency colors.
        backgroundColor: 'rgba(14, 165, 233, 0.08)', // design-governance-allow: raw-color - Chart.js canvas requires fixed latency colors.
        yAxisID: 'yTtft',
        tension: 0.4,
        cubicInterpolationMode: 'monotone' as const,
        fill: false,
...
const chartOptions = computed(() => {
  const text = isDark.value ? '#9ca3af' : '#6b7280' // design-governance-allow: raw-color - Chart.js canvas needs theme-aware computed colors.
  const grid = isDark.value ? '#374151' : '#f3f4f6' // design-governance-allow: raw-color - Chart.js canvas needs theme-aware computed colors.
  const tooltipBg = isDark.value ? '#1f2937' : '#ffffff' // design-governance-allow: raw-color - Chart.js canvas needs theme-aware computed colors.
  const tooltipTitle = isDark.value ? '#f3f4f6' : '#111827' // design-governance-allow: raw-color - Chart.js canvas needs theme-aware computed colors.
  const tooltipBody = isDark.value ? '#d1d5db' : '#4b5563' // design-governance-allow: raw-color - Chart.js canvas needs theme-aware computed colors.
  return {
    responsive: true,
    ...
    scales: {
      ...
      yTtft: {
        ...
        min: 0,
        ticks: {
          color: '#0ea5e9', // design-governance-allow: raw-color - Chart.js canvas requires the latency axis color.
          font: { size: 10 },
          callback: (v: string | number) => formatMonitorMs(Number(v)),
        },
        grid: { display: false },
        title: { display: true, text: t('channelMonitorV2.metrics.ttftP50'), color: '#0ea5e9', font: { size: 11 } }, // design-governance-allow: raw-color - Chart.js canvas requires the latency axis color.
      },
    },
  }
})
