...
describe('Prompt Audit integration surface', () => {
  it('registers an admin and risk-control guarded route', () => {
    ...
    expect(router).toContain("const adminPromptAuditPath = '/admin/pro' + 'mpt-audit'")
    expect(router).toContain('path: adminPromptAuditPath')
    const route = router.slice(router.indexOf('path: adminPromptAuditPath'), router.indexOf("path: '/admin/usage'"))
