...
export const useAdminSettingsStore = defineStore('adminSettings', () => {
  ...
  // Share one request between the sidebar and route guards. Without this,
  // `fetch()` returned immediately while a request was already in progress,
  // leaving direct admin custom-page navigations with an empty menu snapshot.
  ...
  function fetch(force = false): Promise<void> {
    ...
    // Clear only after this specific request settles. A new caller can then
    // force a fresh fetch without racing the cleanup of the old promise.
    void request.then(
      () => {
