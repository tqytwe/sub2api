export default {
  plugins: {
    ...
    untested: '未验证版本',
    incompatible: '不兼容',
    compatibleMessage: '当前 Sub2API 版本已由插件声明测试',
    untestedMessage: '版本范围兼容，但插件未声明已测试当前 Sub2API 版本',
    incompatibleMessage: '当前 Sub2API {current} 不满足插件要求 {required}',
    runtimeReady: '插件进程运行中',
    runtimeUnavailable: '插件进程当前不可用',
    runtimeError: '插件运行失败，请检查服务端日志或重新测试。',
    enabled: '已启用',
    disabled: '已停用',
    error: '异常',
    starting: '启动中',
    ...
    testSuccess: '插件测试通过',
    testFailed: '插件测试失败，请打开配置界面或检查服务端日志了解详情。',
