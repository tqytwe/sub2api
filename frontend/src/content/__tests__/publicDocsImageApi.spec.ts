...
describe('public image API documentation placement', () => {
  ...
  it('documents the three real image response contracts and multipart boundary', () => {
    ...
    expect(asyncPage?.html).toContain('IMAGE_ASYNC_WORKER_COUNT=4')
    expect(asyncPage?.html).toContain('IMAGE_STORAGE_BACKEND=s3')
    expect(asyncPage?.html).toContain('IMAGE_STORAGE_ENDPOINT=https://jisu.zeabur.app')
    expect(asyncPage?.html).toContain('IMAGE_STORAGE_BUCKET=image-task-results')
    expect(asyncPage?.html).toContain('IMAGE_STORAGE_PREFIX=images/')
    expect(asyncPage?.html).toContain('IMAGE_STORAGE_PRESIGN_EXPIRY_HOURS=24')
    expect(asyncPage?.html).toContain('预签 URL 24 小时过期')
    expect(asyncPage?.html).toContain('RustFS 生命周期规则')
    ...
    const placeholderRustFSURL = ['https://your', 'rustfs-api-domain'].join('-')
    expect(asyncPage?.html).not.toContain(placeholderRustFSURL)
