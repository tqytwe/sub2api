<template>
  ...
  <AppLayout v-if="isEmbedded">
    <ModelPlazaContent :response="data" :loading="loading" :error="loadFailed" :family="family" embedded />
  </AppLayout>
  ...
  <div v-else class="bg-gray-50 dark:bg-dark-950">
    ...
    <main class="w-full px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <div class="public-content-frame" data-testid="model-plaza-public-frame">
        <ModelPlazaContent :response="data" :loading="loading" :error="loadFailed" :family="family" />
      </div>
    </main>
  </div>
</template>
...
import { parseModelFamily } from '@/components/modelPlaza/modelFamilies'
...
const family = computed(() => parseModelFamily(route.params.family))
