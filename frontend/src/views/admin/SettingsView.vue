    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.play.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.play.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.play.checkinEnabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.play.checkinEnabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.play_checkin_enabled" />
            </div>
            <div v-if="form.play_checkin_enabled">
              <label class="input-label">{{ t('admin.settings.features.play.checkinDailyReward') }}</label>
              <input v-model.number="form.play_checkin_daily_reward" type="number" step="0.01" min="0" class="input" />
            </div>
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.play.arenaEnabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.play.arenaEnabledHint') }}
                </p>
              </div>
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.play.blindboxEnabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.play.blindboxEnabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.play_blindbox_enabled" />
            </div>
            <div v-if="form.play_blindbox_enabled" class="grid gap-4 sm:grid-cols-2">
              <div>
                <label class="input-label">{{ t('admin.settings.features.play.blindboxCost') }}</label>
                <input v-model.number="form.play_blindbox_cost" type="number" step="0.01" min="0" class="input" />
              </div>
              <div>
                <label class="input-label">{{ t('admin.settings.features.play.blindboxDailyLimit') }}</label>
                <input v-model.number="form.play_blindbox_daily_limit" type="number" min="1" class="input" />
              </div>
            </div>
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.play.quizEnabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.play.quizEnabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.play_quiz_enabled" />
            </div>
            <div v-if="form.play_quiz_enabled" class="grid gap-4 sm:grid-cols-2">
              <div>
                <label class="input-label">{{ t('admin.settings.features.play.quizRewardPerCorrect') }}</label>
                <input v-model.number="form.play_quiz_reward_per_correct" type="number" step="0.01" min="0" class="input" />
              </div>
              <div>
                <label class="input-label">{{ t('admin.settings.features.play.quizQuestionsPerDay') }}</label>
                <input v-model.number="form.play_quiz_questions_per_day" type="number" min="1" max="20" class="input" />
              </div>
            </div>
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.play.agentTeamEnabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.play.agentTeamEnabledHint') }}
                </p>
              </div>
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      play_checkin_daily_reward: Number(form.play_checkin_daily_reward) || 0.5,
      play_arena_enabled: form.play_arena_enabled,
      play_blindbox_enabled: form.play_blindbox_enabled,
      play_blindbox_cost: Number(form.play_blindbox_cost) || 0.5,
      play_blindbox_daily_limit: Number(form.play_blindbox_daily_limit) || 10,
      play_quiz_enabled: form.play_quiz_enabled,
      play_quiz_reward_per_correct: Number(form.play_quiz_reward_per_correct) || 0.1,
      play_quiz_questions_per_day: Number(form.play_quiz_questions_per_day) || 5,
