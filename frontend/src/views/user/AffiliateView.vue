    ...
    <div class="space-y-6">
      ...
      <template v-else-if="detail">
        ...
        <div class="card p-6">
          ...
          <p v-if="teamInviteCode" class="mt-3 text-sm text-gray-500 dark:text-dark-400">
            {{ t('affiliate.teamLinkHint', { code: teamInviteCode }) }}
          </p>
...
const teamInviteCode = ref<string | null>(null)

const inviteLink = computed(() => {
  if (!detail.value) return ''
  return buildRegisterInviteLink(detail.value.aff_code, teamInviteCode.value ?? undefined)
})
...
async function loadAffiliateDetail(silent = false): Promise<void> {
  ...
  try {
    const [affiliateDetail, teamMe] = await Promise.all([
      userAPI.getAffiliateDetail(),
      playAPI.getTeamMe().catch(() => null),
    ])
    detail.value = affiliateDetail
    const team = teamMe?.team
    if (team && team.captain_id === authStore.user?.id) {
      teamInviteCode.value = team.invite_code
    } else {
      teamInviteCode.value = null
    }
  } catch (error) {
