...
export function parseWechatResumeRoute(
  ...
  const couponId = Number.parseInt(readQueryString(query, 'coupon_id'), 10)
  const hasCouponId = Number.isFinite(couponId) && couponId > 0
  ...
  return {
    ...
    orderAmount,
    planId: hasPlanId ? planId : undefined,
    couponId: hasCouponId ? couponId : undefined,
  }
}
